...
omega = 1       #Generally controls how fast the points move, specifically, multiplies globally into each input speed
...
offsets = [5.91, 5.8, 3.14 * 1.5, 4.5, 4.84, 6.19, 4.63, 2.11] #randomly generated except earth
...
for i in range(1200):
	...
	writeTexture(getPosition(radii[4], t, periods[4], offsets[4]), 16, 4) #Jupiter
	writePoint(getPosition(radii[5], t, periods[5], offsets[5]), 3, 100, 5) #Saturn
	writePoint(getPosition(radii[6], t, periods[6], offsets[6]), 4, 90, 5) #Uranus
	writePoint(getPosition(radii[7], t, periods[7], offsets[7]), 5, 90, 5) #Neptune
